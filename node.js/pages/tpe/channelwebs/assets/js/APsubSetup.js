...
function setupAirbox(mid, postCode, areaCode, action) {//OK
    ...
    switch (action) {
        ...
        //url: API_HOST + SRU + '/listSubscriptionContainer/',
            $.ajax({
                url:  SRU + '/listSubscriptionContainer/',
                data: {
                    'memberId': mid,
                    'datasetId': 'airbox',
...
function updateAirboxSubArea(sel, action) {//OK
    var desc = document.getElementById('subc__desc');
    ...
    if (checkedBoxes.length === 0) {
        ...
        $.ajax({
            ...
            document.getElementById('content').innerHTML = '<button class="sbtn--want" onclick="location.href=\'/login/airbox\'">繼續訂閱其他區域</button>';
        }).fail(function(jqXhr, text, et) {});
    }
}

function addAirboxSub(sel) {//OK
    var desc = document.getElementById('subc__desc');
    var checkedBoxes = document.querySelectorAll('.aircheck:checked');
    if (checkedBoxes.length === 0) {
        alert('至少選 1 個時段');
